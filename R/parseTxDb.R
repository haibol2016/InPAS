...
#' @author Haibo Liu
#' @return a tibble which can be converted to an object of
...
assign_feature <- function(gr, feature_alt = "utr3") {
  gr <- gr %>%
    ...
    dplyr::mutate(feature = ifelse(!duplicated(paste0(
      seqnames,
      "|", transcript
    )),
    feature_alt, feature
...
#'
#' parsed_Txdb <- parseTxDb(TxDb, edb, removeScaffolds = TRUE)
parseTxDb <- function(TxDb = NULL, edb = NULL,
  ...
  multiplex <- multiplex[unique(noncoding_exons_multiplex$transcript), , drop = F]
